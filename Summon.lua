local addonName, QOLUtilsCommon = ...

QOLUtilsCommon.SMN = {}
local smn = QOLUtilsCommon.SMN
...
function smn.Pet()
	local onlyFavorites = QOLUtilsCommon.SettingIsTrue(QOL_Config_Acct.SMN.OnlyFavoritePets, QOL_Config_Toon.SMN.OnlyFavoritePets)
...
function smn.Mount()
	if IsMounted() then
		...
		elseif inAhnqiraj then
			...
			if QOLUtilsCommon.TableIsNilOrEmpty(usableMounts) then
				usableMounts = smn.ScanJournal(usableMounts, smn.Types.Ground)
			end
		else
			usableMounts = smn.ScanJournal(usableMounts, smn.Types.Ground)
		end
		if QOLUtilsCommon.TableIsNilOrEmpty(usableMounts)
				and QOLUtilsCommon.SettingIsTrue(QOL_Config_Acct.SMN.OnlyFavoriteMounts, QOL_Config_Toon.SMN.OnlyFavoriteMounts) then
			if lowLevel then
...
function smn.ScanJournal(existingMounts, validTypeA, validTypeB)
	...
	local onlyFavorites = QOLUtilsCommon.SettingIsTrue(QOL_Config_Acct.SMN.OnlyFavoriteMounts, QOL_Config_Toon.SMN.OnlyFavoriteMounts)
...
function smn.ToggleFavoritePets(state)
	QOL_Config_Acct.SMN.OnlyFavoritePets, QOL_Config_Toon.SMN.OnlyFavoritePets =
	QOLUtilsCommon.ToggleSetting(state,
		QOL_Config_Acct.SMN.OnlyFavoritePets,
		QOL_Config_Toon.SMN.OnlyFavoritePets,
		QOLUtilsCommon.OPT.Acct.SMN.CheckBoxPets,
		QOLUtilsCommon.OPT.Toon.SMN.CheckBoxPets)
end

function smn.ToggleFavoriteMounts(state)
	QOL_Config_Acct.SMN.OnlyFavoriteMounts, QOL_Config_Toon.SMN.OnlyFavoriteMounts =
	QOLUtilsCommon.ToggleSetting(state,
		QOL_Config_Acct.SMN.OnlyFavoriteMounts,
		QOL_Config_Toon.SMN.OnlyFavoriteMounts,
		QOLUtilsCommon.OPT.Acct.SMN.CheckBoxMounts,
		QOLUtilsCommon.OPT.Toon.SMN.CheckBoxMounts)
end

function smn.ToggleLogonReport()
	QOL_Config_Acct.SMN.ReportAtLogon, QOL_Config_Toon.SMN.ReportAtLogon =
	QOLUtilsCommon.ToggleSetting(nil,
		QOL_Config_Acct.SMN.ReportAtLogon,
		QOL_Config_Toon.SMN.ReportAtLogon,
		QOLUtilsCommon.OPT.Acct.SMN.CheckBoxReport,
		QOLUtilsCommon.OPT.Toon.SMN.CheckBoxReport)
end

function smn.ReportFavoritePets()
	if QOLUtilsCommon.SettingIsTrue(QOL_Config_Acct.SMN.OnlyFavoritePets, QOL_Config_Toon.SMN.OnlyFavoritePets) then
...
function smn.ReportFavoriteMounts()
	if QOLUtilsCommon.SettingIsTrue(QOL_Config_Acct.SMN.OnlyFavoriteMounts, QOL_Config_Toon.SMN.OnlyFavoriteMounts) then
...
function smn.ReportInitial()
	if QOLUtilsCommon.SettingIsTrue(QOL_Config_Acct.SMN.ReportAtLogon, QOL_Config_Toon.SMN.ReportAtLogon) then
...
function smn.ToggleFavoritePetsOnClick()
	QOL_Config_Acct.SMN.OnlyFavoritePets = QOLUtilsCommon.OPT.Acct.SMN.CheckBoxPets:GetChecked()
	QOL_Config_Toon.SMN.OnlyFavoritePets = QOLUtilsCommon.OPT.Toon.SMN.CheckBoxPets:GetChecked()
end

function smn.ToggleFavoriteMountsOnClick()
	QOL_Config_Acct.SMN.OnlyFavoriteMounts = QOLUtilsCommon.OPT.Acct.SMN.CheckBoxMounts:GetChecked()
	QOL_Config_Toon.SMN.OnlyFavoriteMounts = QOLUtilsCommon.OPT.Toon.SMN.CheckBoxMounts:GetChecked()
end

function smn.ToggleLogonReportOnClick()
	QOL_Config_Acct.SMN.ReportAtLogon = QOLUtilsCommon.OPT.Acct.SMN.CheckBoxReport:GetChecked()
	QOL_Config_Toon.SMN.ReportAtLogon = QOLUtilsCommon.OPT.Toon.SMN.CheckBoxReport:GetChecked()
end

function smn.Log(msg)
	QOLUtilsCommon.Log(msg, 'SMN')
end